  ...
  <div class="row">
    <ul class="filter-bar">
      <li class="search-input">
        <input type="text" placeholder="search movie or Artist" v-model="searchTermInp" @input="searchMovie('instant')">
        <i class="fas fa-search" @click="searchMovie('full')"></i>
        <results-popup v-if="showResultsPopup"
          :movies="shortSearchResults"
          @close="hideSearchResult"
...
<script>
  ...
  import ResultsPopup from './SearchResultsPopup.vue';
  export default{
    components:{
      FilterItems,
      ResultsPopup,
    },
    emits:['filter-movies', 'search-movie','search-movie-instant'],
    data(){
      return{
        // sortMode:'',
        sortTerm:'',
    ...
    methods:{
      ...
      async searchMovie(type='full'){
        ...
        if(type === 'full'){
          ...
          const dataLen = responseData.results.length;
          if(dataLen >0){
            this.showResultsPopup = true;
          }
          //Get movie search(by text) results to show on popup
          //Slice results to 5 nos
          const shortResult = responseData.results.slice(0,5);
          this.shortSearchResults = shortResult;
          console.log('shortResult',shortResult);
          //Emit instant results to MoviesList
          this.$emit('search-movie-instant', 'search',responseData.results);
        }
